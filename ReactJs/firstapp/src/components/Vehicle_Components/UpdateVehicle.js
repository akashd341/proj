...
import App from "../../App";
import CompleteVech from "./CompleteVech";
...
// })
    ...
    return(
        ...
        <h1 className='text-2xl font-medium text-primary mt-2 text-center'>
           ...
           <li className="mt-2">Driver_id : {deldata[0].driver_id}</li>
           
           
           </CardContent>
           <button onClick={updat} type="button" class=" inline-block px-3 py-2 ml-[33%] bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Update</button>
         
        </Card>
        </div>
        :null}
        </Container>
        {check?<CompleteVech val={deldata} ck={stcheck}/>:null}
        </>
    )
}
